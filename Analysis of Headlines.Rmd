...
description: |
  Assignment 8 for DACSS 697D Course 'Text as Data': "Putting it All Together"
categories:
  - text as data
  - homework
...
headlines_main <- read.csv("./data/afghanistan_withdrawal_main.csv")
headlines_print <- read.csv("./data/afghanistan_withdrawal_print.csv")
headlines_both <- read.csv("./data/afghanistan_withdrawal_headlines.csv")
...
```{r code_folding = TRUE}
main_corpus <- corpus(headlines_main, docid_field = "article_id", text_field = "headline_main")
print_corpus <- corpus(headlines_print, docid_field = "article_id", text_field = "headline_print")
```
...
After many process posts, I finally realized how to remove the "�" symbol that has plagued me since starting working with this API by using "remove_symbols=TRUE" in addition to removing the punctuation when tokenizing. I also want to remove stopwords.
