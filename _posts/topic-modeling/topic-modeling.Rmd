...
The wordcloud is a good preliminary way to look at the topics. I'll choose topic "5", because in all three sample documents, topic "5" seems to be the most broad capture of the headline topic prevalence over the time period.
...
The wordcloud is a good preliminary way to look at the topics. But it is clearly more difficult to know the overall topic model prevalence with the lower alpha and its more granular results.

```{r code_folding=TRUE}
# visualize topics as word cloud
topicToVizb <- 2 # change for your own topic of interest
#topicToViz <- grep('fear', topicNames)[1] # Or select a topic by a term contained in its name
# select to 40 most probable terms from the topic by sorting the term-topic-probability vector in decreasing order
...
The fact that a topic model conveys of topic probabilities for each document, resp. paragraph in our case, makes it possible to use it for thematic filtering of a collection. AS filter we select only those documents which exceed a certain threshold of their probability value for certain topics (for example, each document which contains the topic "Withdrawal" to more than 20 percent).

```{r code_folding=TRUE}
#topicToFilter <- 2  # you can set this manually ...
...
The wordcloud is a good preliminary way to look at the topics. I'll look at topic 2, though in this model the topics are all distributed so broadly this doesn't help much in this case.
...
Changing the alpha to the lower level from 0.3019 to 0.1

```{r code_folding=TRUE}
# see alpha from previous model
attr(topicModel2, "alpha") 

topicModel2b <- LDA(DTM, K2, method="VEM", control=list(alpha = 0.1))
```
...
```{r code_folding=TRUE}
#visualize topics as word cloud
topicToViz2b <- 2 # change for your own topic of interest
#topicToViz <- grep('fear', topicNames)[1] # Or select a topic by a term contained in its name
#select to 40 most probable terms from the topic by sorting the term-topic-probability vector in decreasing order
...
```{r code_folding=TRUE}
#topicToFilter2 <- 3  # you can set this manually ...
# ... or have it selected by a term in the topic name (e.g. 'children')
